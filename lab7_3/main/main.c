...
#define WTTR_PORT 		"80"
#define WTTR_LOCATION		"Santa-Cruz-USA"
//#define WTTR_PATH 		"/Santa-Cruz-USA?m&format=%t"
...
static const char *TAG = "ESP32-Weather-Station";

char* path_string = "/%s?format=%%t&m";
...
char wttr_confirmation[128] = "";
char outside_temp[128] = "";
static const char *REQUEST;
static const char *POST_PAYLOAD;
static const char *TEMPLATE;
char WTTR_PATH[256] = "";

void init_wttr() {
    int wttr_err = snprintf(WTTR_PATH, sizeof(WTTR_PATH), path_string, WTTR_LOCATION);
    if (wttr_err < 0) {
	ESP_LOGE(TAG, "Unable to get path. Please format location correctly");
    }
    ...
    snprintf(request_buf, sizeof(request_buf),
         ...
         "\r\n",
         WTTR_PATH, WTTR_SERVER, WTTR_PORT);
    REQUEST = request_buf;
    POST_PAYLOAD = "| Outside Temperature (courtesy of wttr.in): %s |\n| Inside Temperature: %d°C (%d°F), Humidity: %d%% |\n| Location: %s |";
    TEMPLATE = "POST " WEB_PATH " HTTP/1.0\r\n"
...
static void http_get_wttr_weather(void *pvParameters)
{
    ...
    while(1) {
        int err = getaddrinfo(WTTR_SERVER, WTTR_PORT, &hints, &res);
        ...
        do {
            ...
            for(int i = 0; i < r; i++) {
                putchar(recv_buf[i]);
    		memset(wttr_confirmation, 0, 128);
		strcat(wttr_confirmation, recv_buf);
            }
        } while(r > 0);

	// Credit to cnicutar on StackOverflow 'https://stackoverflow.com/questions/9219940/traversing-c-string-get-the-last-word-of-a-string'
	memset(outside_temp, 0, 128);
	char* temp = strrchr(wttr_confirmation, '\n');
	if (temp[0] == '\n') {
	    memmove(temp, temp + 1, strlen(temp)); // Shift the string left
	}
	strcat(outside_temp, temp);
...
static void http_post_task(void *pvParameters)
{
	...
	char payload[256];
    	int ret = snprintf(payload, sizeof(payload), POST_PAYLOAD, outside_temp, (int)temp_celsius, (int)temp_fahrenheit, (int)humidity, WTTR_LOCATION);
    	if (ret < 0)
...
void app_main(void)
{
    ...
    xTaskCreate(&http_get_wttr_weather, "http_get_wttr_weather", 4096, NULL, 5, NULL);
